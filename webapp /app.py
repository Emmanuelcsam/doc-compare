# app.py - Flask webapp for document comparison
...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            paras1 = [p.strip() for p in text1.replace('\n\n', '\n¶\n').split('\n¶\n') if p.strip()] or [p.strip() for p in text1.split('\n') if p.strip()]
            paras2 = [p.strip() for p in text2.replace('\n\n', '\n¶\n').split('\n¶\n') if p.strip()] or [p.strip() for p in text2.split('\n') if p.strip()]
            
            sum1 = generate_summary(text1, summary_sentences)
            sum2 = generate_summary(text2, summary_sentences)
            
            # Summaries with sources and entities
            sum1_data = []
            for s in sum1:
                source = find_paragraph_source(s, paras1)
                doc = nlp(s)
                entities = ', '.join([ent.text for ent in doc.ents]) if doc.ents else 'None'
                sum1_data.append({'text': s, 'source': source, 'entities': entities})
            ...
            for s in sum2:
                ...
                doc = nlp(s)
                entities = ', '.join([ent.text for ent in doc.ents]) if doc.ents else 'None'
                sum2_data.append({'text': s, 'source': source, 'entities': entities})
            ...
            unique1 = [(i+1, paras1[i], sent1[i]) for i in range(len(paras1)) if max(sim_matrix[i]) < similarity_threshold]
            unique2 = [(j+1, paras2[j], sent2[j]) for j in range(len(paras2)) if max(sim_matrix[:, j]) < similarity_threshold]
